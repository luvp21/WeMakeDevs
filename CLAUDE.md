...
6. **Video rendering does not run on Lambda.** It runs on Fargate. The Fargate image only changes when it is rebuilt and pushed, so after any change under `render/` or `shared/` rebuild it (or test with `RENDER_MODE=local`).
...
8. **The Zod schemas in `shared/` are the single source of truth.** Change a shape there first; the API, the frontend and the renderer all use it.
